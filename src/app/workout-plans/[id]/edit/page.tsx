...
export default function EditWorkoutPlanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, saveMode: 'save' | 'assign') => {
    e.preventDefault()
    setLoading(true)
    try {
      const payload: any = {
        ...
        status: saveMode === 'save' ? 'draft' : status,
        days,
        notes,
      }
      ...
      <div>
        ...
        <form className="space-y-6">
          <div className="card p-6 grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="label">Client * <span className="text-[var(--text-tertiary)] font-normal">(leave empty to save without assigning)</span></label>
                  ...
                  {day.exercises.map((ex, ei) => (
                    <div key={ei} className="p-3 grid grid-cols-12 gap-2 items-start">
                      ...
                      <div className="col-span-3">
                        <input value={(ex as any).video_url || ''} onChange={e => updateExercise(di, ei, 'video_url', e.target.value)}
                               className="input text-sm" placeholder="Video URL (YouTube, Vimeo...)" />
                      </div>
