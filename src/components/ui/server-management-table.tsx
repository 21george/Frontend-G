...
export function ServerManagementTable({
  ...
  const getOSIcon = (osType: Server["osType"]) => {
    switch (osType) {
      case "windows":
        return (
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center p-1.5 border border-[var(--border)]/30">
      ...
      case "ubuntu":
        return (
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center p-1.5 border border-[var(--border)]/30">
      ...
      case "linux":
        return (
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gray-600 to-gray-800 flex items-center justify-center border border-[var(--border)]/30">
  ...
  const getCPUBars = (percentage: number, status: Server["status"]) => {
    ...
    const getBarColor = (index: number) => {
      ...
      switch (status) {
        case "active":
          return isDark ? "bg-[var(--text-primary)]/60" : "bg-[var(--text-primary)]/60";
        case "paused":
          return isDark ? "bg-[var(--text-secondary)]/50" : "bg-[var(--text-secondary)]/50";
        case "inactive":
          return isDark ? "bg-[var(--text-secondary)]/30" : "bg-[var(--text-secondary)]/30";
        default:
          return isDark ? "bg-[var(--text-primary)]/60" : "bg-[var(--text-primary)]/60";
      }
    };
    ...
    return (
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-mono text-[var(--text-primary)] font-medium min-w-[3rem]">
          {percentage}%
        </span>
      </div>
    );
  ...
  const getStatusBadge = (status: Server["status"]) => {
    switch (status) {
      case "active":
        return (
          <div className="px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center">
            <span className="text-emerald-500 text-sm font-medium">Active</span>
          </div>
        );
      case "paused":
        return (
          <div className="px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 flex items-center justify-center">
  ...
  return (
    <div className={`w-full max-w-7xl mx-auto p-6 ${className}`}>
      <div className="relative border border-[var(--border)] rounded-2xl p-6 bg-[var(--bg-card)]">
        {/* Header */}
        ...
        >
          ...
          {servers.map((server) => (
            ...
            >
              <motion.div
                className="relative bg-[#13131314] dark:bg-white/[0.03] border border-[var(--border)]/50 rounded-xl p-4 overflow-hidden"
                whileHover={
                  shouldReduceMotion
                    ? {}
                    : {
              ...
              >
                ...
                <div className="relative grid grid-cols-12 gap-4 items-center">
                  ...
                  {/* Service Location */}
                  <div className="col-span-2 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full overflow-hidden border border-[var(--border)]/30 flex items-center justify-center">
                  ...
                  {/* Due Date */}
                  <div className="col-span-2">
                    <span className="text-[var(--text-primary)]">{server.dueDate}</span>
                  </div>
        ...
        <AnimatePresence>
          {selectedServer && (
            <motion.div
              ...
              className="absolute inset-0 bg-[var(--bg-page)]/60 backdrop-blur-sm flex flex-col rounded-2xl z-10 overflow-hidden"
            >
              {/* Header with Actions */}
              <div className="relative bg-gradient-to-r from-slate-50/50 dark:from-white/[0.03] to-transparent p-4 border-b border-[var(--border)]/30 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full overflow-hidden border border-[var(--border)]/30 flex items-center justify-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedServer.status === "active" ? (
                    ...
                    <motion.button
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-lg text-sm transition-colors"
                  ...
                  {selectedServer.status === "paused" ? (
                    ...
                    <motion.button
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-lg text-sm transition-colors"
                  ...
                  {/* Close Button */}
                  <motion.button
                    className="w-8 h-8 bg-[var(--bg-card)]/80 hover:bg-[var(--bg-card)] rounded-full flex items-center justify-center border border-[var(--border)]/50 ml-2"
              ...
              <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {/* IP Address */}
                  <div className="bg-slate-50/40 dark:bg-white/[0.02] rounded-lg p-3 border border-[var(--border)]/30">
                ...
                <div className="bg-slate-50/40 dark:bg-white/[0.02] rounded-lg p-3 border border-[var(--border)]/30">
                  ...
                  <div className="font-mono text-xs space-y-1 max-h-24 overflow-y-auto">
                    <div className="text-emerald-500">[15:42:31] Server started successfully</div>
                    <div className="text-blue-400">[15:42:25] System health check passed</div>
                    <div className="text-amber-400">[15:41:18] CPU usage: {selectedServer.cpuPercentage}%</div>
                    <div className="text-[var(--text-secondary)]">[15:40:05] Connection from {selectedServer.ip}</div>
                  </div>
                </div>
              </div>
            </motion.div>
