  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setCalDate(d => subMonths(d, 1))} className="p-1.5 hover:bg-[#13131314] dark:hover:bg-white/[0.06] transition-colors rounded-lg">
          <ChevronLeft size={14} className="text-[var(--text-tertiary)] dark:text-slate-400" />
        </button>
        <span className="text-sm font-semibold text-[var(--text-primary)] dark:text-slate-100">
          {format(calDate, 'MMMM yyyy')}
        </span>
        <button onClick={() => setCalDate(d => addMonths(d, 1))} className="p-1.5 hover:bg-[#13131314] dark:hover:bg-white/[0.06] transition-colors rounded-lg">
          <ChevronRight size={14} className="text-[var(--text-tertiary)] dark:text-slate-400" />
        </button>
      </div>

      <div className="grid grid-cols-7 gap-y-1 mb-2">
        {['M','T','W','T','F','S','S'].map((d, i) => (
          <div key={`${d}-${i}`} className="text-center text-[10px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)] dark:text-slate-400">{d}</div>
        ))}
      </div>
          ...
          return (
            <motion.button
              ...
              className={`w-9 h-9 text-xs font-medium flex items-center justify-center transition-colors relative mx-auto rounded-full
                ...
                ${!isTodayDay && !isActive && isCurrent ? 'hover:bg-[#13131314] dark:hover:bg-white/[0.06] text-[var(--text-primary)] dark:text-slate-100' : ''}
              `}
            >
              {day.getDate()}
              {hasEvent && !isActive && (
...
export function EventTypeFilter({ selectedTypes, onToggleType, checkins }: EventTypeFilterProps) {
  return (
    <div className="mb-8">
      <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--text-tertiary)] dark:text-slate-400 mb-4">Event Types</h3>
          ...
          return (
            <motion.button
              ...
              className={`w-full flex items-center gap-3 cursor-pointer p-3 transition-all rounded-lg ${
                isSelected ? 'bg-[var(--bg-card)] dark:bg-surface-subtle-dark shadow-sm' : 'hover:bg-[#13131314] dark:hover:bg-white/[0.02]'
              }`}
            >
              <div className={`w-4 h-4 flex items-center justify-center transition-colors rounded ${colors.iconBg}`}>
                {isSelected && <CheckCircle2 size={10} className="text-white" />}
              </div>
              <span className={`text-sm capitalize ${isSelected ? 'text-[var(--text-primary)] dark:text-slate-100 font-medium' : 'text-[var(--text-tertiary)] dark:text-slate-400'}`}>
                {key}
              </span>
...
export function TodaysSchedule({ checkins, clientMap, onSelectEvent }: TodaysScheduleProps) {
  ...
  return (
    <div>
      <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--text-tertiary)] dark:text-slate-400 mb-4">Today</h3>
      <div className="space-y-3">
        {todayEvents.slice(0, 4).map((event, i) => {
          ...
          return (
            <motion.div
              ...
              className="group p-3 bg-[var(--bg-card)] dark:bg-surface-subtle-dark border border-[var(--border)] dark:border-white/[0.06] hover:border-brand-300 dark:hover:border-brand-700 transition-all cursor-pointer rounded-lg"
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--text-primary)] dark:text-slate-100 truncate">{client?.name ?? 'Client'}</p>
                  {time && (
                    <p className="text-[11px] text-[var(--text-tertiary)] dark:text-slate-400">{format(time, 'h:mm a')}</p>
                  )}
                </div>
        ...
        {todayEvents.length === 0 && (
          <div className="text-center py-6">
            ...
            <p className="text-xs text-[var(--text-tertiary)] dark:text-slate-400">No events today</p>
          </div>
        )}
      </div>
    </div>
