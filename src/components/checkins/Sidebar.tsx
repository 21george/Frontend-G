    ...
    <div className="mb-8">
      <div className="flex items-center justify-between mb-4">
        <button
          ...
          className="p-1.5 hover:bg-[#13131314] dark:hover:bg-white/[0.06] transition-colors rounded-lg"
        >
          <ChevronLeft
            size={14}
            className="text-[var(--text-tertiary)] dark:text-slate-400"
          />
        </button>
      ...
      <div className="grid grid-cols-7 gap-y-1 mb-2">
        {["M", "T", "W", "T", "F", "S", "S"].map((d, i) => (
          <div
            key={`${d}-${i}`}
            className="text-center text-[10px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)] dark:text-slate-400"
          >
            {d}
          </div>
        ))}
...
export function EventTypeFilter({
  ...
  return (
    <div className="mb-8">
      <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--text-tertiary)] dark:text-slate-400 mb-4">
        Event Types
      </h3>
          ...
          return (
            <motion.button
              ...
              className={`w-full flex items-center gap-3 cursor-pointer p-3 transition-all rounded-lg ${
                isSelected
                  ? "bg-[var(--bg-card)] dark:bg-surface-subtle-dark shadow-sm"
                  : "hover:bg-[var(--bg-hover)] dark:hover:bg-white/[0.02]"
              }`}
            >
              <div
                className={`w-4 h-4 flex items-center justify-center transition-colors rounded ${colors.iconBg}`}
              >
                {isSelected && (
              ...
              <span
                className={`text-sm capitalize ${isSelected ? "text-[var(--text-primary)] dark:text-slate-100 font-medium" : "text-[var(--text-tertiary)] dark:text-slate-400"}`}
              >
                {key}
              </span>
              <span
                className={`ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded ${colors.bg} ${colors.text}`}
              >
      ...
      <div className="space-y-3">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5">
                <div
                  className={`w-8 h-8 flex items-center justify-center rounded ${colors.iconBg}`}
                >
                  {event.type === "video" && (
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--text-primary)] dark:text-slate-100 truncate">
                    {client?.name ?? "Client"}
                  </p>
                  {time && (
                    <p className="text-[11px] text-[var(--text-tertiary)] dark:text-slate-400">
                      {format(time, "h:mm a")}
                    </p>
                  )}
                </div>
        ...
        {todayEvents.length === 0 && (
          <div className="text-center py-6">
            ...
            <p className="text-xs text-[var(--text-tertiary)] dark:text-slate-400">
              No events today
            </p>
          </div>
        )}
