...
import type { AnalyticsData, Client } from "@/types";
...
function MeasureBadge({
  ...
  const diff =
    value != null && prev != null ? +(value - prev).toFixed(1) : null;
  return (
    <div className="p-3 border border-[var(--border)] dark:border-white/[0.06] bg-[var(--bg-subtle)] dark:bg-white/[0.02]">
      ...
      {diff != null && (
        <p
          className={`text-[10px] font-medium mt-0.5 ${diff < 0 ? "text-emerald-500" : diff > 0 ? "text-rose-400" : "text-slate-400"}`}
        >
          {diff > 0 ? "+" : ""}
          {diff} {unit} vs prev
        </p>
  ...
  return (
    <div className="border border-[var(--border)] dark:border-white/[0.07] bg-[var(--bg-card)] p-4 flex items-start gap-3">
      ...
      <div className="min-w-0">
        <p className="text-[10px] font-semibold text-[var(--text-tertiary)] uppercase tracking-widest mb-0.5">
          {label}
        </p>
        <p
...
/* ─── Main component ────────────────────────────────────────────────────── */
export function ClientAnalyticsTab({ analytics, client }: Props) {
  const gender = client?.gender === "female" ? "female" : "male";
  const completionRates = Array.isArray(analytics?.completion_rate)
    ? analytics.completion_rate
    : [];
  const weeklyVolume = Array.isArray(analytics?.weekly_volume)
    ? analytics.weekly_volume
    : [];
  const personalRecords = Array.isArray(analytics?.personal_records)
    ? analytics.personal_records
    : [];
  const exerciseProgress = analytics?.exercise_progress ?? {};
  const measurements = Array.isArray(analytics?.measurements)
  ...
  const avgCompletion = useMemo(() => {
    ...
    return Math.round(
      completionRates.reduce((s, w) => s + w.rate, 0) / completionRates.length,
    );
  }, [completionRates]);
        ...
        {latestM == null ? (
          ...
          <div className="flex flex-col sm:flex-row gap-5 items-center sm:items-start">
            ...
            <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 gap-2.5 w-full">
              <MeasureBadge
                label="Weight"
                value={latestM.weight_kg}
                prev={prevM?.weight_kg}
                unit="kg"
                dotColor="#6366f1"
              />
              <MeasureBadge
                label="Chest"
                value={latestM.chest_cm}
                prev={prevM?.chest_cm}
                unit="cm"
                dotColor="#3b82f6"
              />
              ...
              <MeasureBadge
                label="Body Fat"
                value={latestM.body_fat_pct}
                prev={prevM?.body_fat_pct}
                unit="%"
                dotColor="#8b5cf6"
              />
              {latestM.date && (
                <div className="p-3 border border-[var(--border)] dark:border-white/[0.06] bg-[var(--bg-subtle)] dark:bg-white/[0.02] flex flex-col justify-center">
                  ...
                  {measurements.length > 1 && (
                    <p className="text-[10px] text-[var(--text-tertiary)] mt-0.5">
                      {measurements.length} check-ins total
                    </p>
                  )}
                </div>
              )}
          ...
          <>
            ...
            <div className="flex gap-1 mt-1.5">
              {completionRates.slice(-12).map((w, i) => (
                <div
                  key={i}
                  className="flex-1 text-[9px] text-center text-slate-400 dark:text-slate-600 truncate"
                >
                  {w.week?.slice(5)}
                </div>
              ))}
            ...
            <div className="flex items-center gap-4 mt-3 flex-wrap">
              {(
                [
                  ["#10b981", "≥80% on track"],
                  ["#f59e0b", "50–79% improving"],
                  ["#ef4444", "<50% needs attention"],
                ] as [string, string][]
              ).map(([c, l]) => (
                <span
                  key={l}
        ...
        <div className="border border-[var(--border)] dark:border-white/[0.07] bg-[var(--bg-card)] p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {personalRecords.map((pr, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-2.5 bg-[var(--bg-subtle)] dark:bg-white/[0.02] border border-[var(--border)] dark:border-white/[0.04]"
              >
          ...
          <div className="space-y-4">
            {Object.entries(exerciseProgress)
              ...
              .map(([name, data]) => {
                ...
                const improvement =
                  latest && first
                    ? +(latest.max_kg - first.max_kg).toFixed(1)
                    : 0;
                return (
                  <div key={name}>
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-[12px] font-medium text-slate-700 dark:text-slate-300 truncate mr-2">
                        {name}
                      </p>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {improvement !== 0 && (
                          <span
                            className={`text-[10px] font-semibold ${improvement > 0 ? "text-emerald-500" : "text-rose-400"}`}
                          >
                            {improvement > 0 ? "+" : ""}
                            {improvement} kg
                          </span>
        ...
        <div className="border border-[var(--border)] dark:border-white/[0.07] bg-[var(--bg-card)] p-5">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-6 gap-2">
            {analytics.photos.map((p) => (
              <div
                key={p.id}
                className="aspect-square border border-[var(--border)] dark:border-white/[0.06] overflow-hidden bg-[var(--bg-subtle)]"
              >
                {p.url ? (
                  <img
                    src={p.url}
