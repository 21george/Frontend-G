...
export function UpcomingSessions({
  ...
  return (
    <motion.div
      ...
      className="xl:col-span-2 bg-[var(--bg-card)] border border-[var(--border)] dark:border-white/[0.07] flex flex-col shadow-[var(--shadow-sm)]"
    >
      ...
      <div className="flex-1 divide-y divide-[var(--border)] dark:divide-white/[0.06]">
        {upcoming.length === 0 ? (
          ...
          upcoming.map((session, i) => {
            ...
            const respondedAt = session.client_responded_at
              ? new Date(session.client_responded_at)
              : null;
            const nowTime = new Date();
            const twentyFourHoursAgo = new Date(
              nowTime.getTime() - 24 * 60 * 60 * 1000,
            );
            const isNew =
              respondedAt !== null && respondedAt > twentyFourHoursAgo;
            return (
              <motion.div
                key={session.id}
