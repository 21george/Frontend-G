...
export const messagesApi = {
  ...
  uploadMedia: (file: File) => {
    ...
    return api.post('/messages/upload-media', formData).then(r =>
      r.data.data as { media_url: string; media_type: string; media_filename: string }
    )
  },

  /** Fetch actual unread messages with client details and content preview */
  getUnreadMessages: () =>
    api.get<{ data: { messages: UnreadMessage[]; count: number } }>('/messages/unread').then(r => r.data),

  /** Mark all client messages as read for the coach */
  markAllRead: () =>
    api.post<{ data: { marked: number } }>('/messages/read-all').then(r => r.data),

  /** Get unread messages count from clients (for notification badge) */
  getUnreadCount: () =>
    api.get<{ data: { count: number; clients: UnreadClientMessage[] } }>('/messages/unread-count').then(r => r.data),
}
